...
export async function loadPlatformData(accessToken) {
  ...
  return {
    ...
    memberships: (viewerSubscriptions.data || []).map((item) => ({ profileId: item.profile_id, tier: item.plan_id, creatorPlan: item.creator_plan_id, status: item.status, renewAt: item.renew_at })),
    unlockedContent,
    ordersHistory,
    platformConfig: PLATFORM_CONFIG,
    mode: 'real',
